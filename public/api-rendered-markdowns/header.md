...
```
http://api.url-shortener.dev/endpoint?limit=100
```
...
```
http://api.url-shortener.dev/endpoint?limit=100&page=2
```
...
```
http://api.url-shortener.dev/endpoint?orderBy=created_at
```
...
```
http://api.url-shortener.dev/endpoint?orderBy=name&sortedBy=desc
```
...
```
http://api.url-shortener.dev/endpoint?search=keyword here
```
...
```
http://api.url-shortener.dev/endpoint?search=first keyword;second keyword
```

#### Search in specific field:
```
http://api.url-shortener.dev/endpoint?search=field:keyword here
```

#### Search in specific fields for multiple keywords: 
```
http://api.url-shortener.dev/endpoint?search=field1:first field keyword;field2:second field keyword
```

#### Define query condition:

```
http://api.url-shortener.dev/endpoint?search=field:keyword&searchFields=name:like
```
...
```
http://api.url-shortener.dev/endpoint?search=field1:first keyword;field2:second keyword&searchFields=field1:like;field2:=;
```
...
```
http://api.url-shortener.dev/endpoint?filter=id;status
```
...
```
http://api.url-shortener.dev/endpoint?page=200
```
...
```shell
  ...
  "meta": {
    "pagination": {
      ...
      "total_pages": 1111,
      "links": {
        "previous": "http://http://api.url-shortener.dev/endpoint?page=21"
      }
    }
  }
```
...
```
http://api.url-shortener.dev/endpoint?include=relationship
```
...
```
http://api.url-shortener.dev/endpoint?skipCache=true
```
...
```shell
curl -X POST -H "Accept: application/json" -H "Content-Type: application/x-www-form-urlencoded; -F "email=admin@admin.com" -F "password=admin" -F "=" "http://http://api.url-shortener.dev/v2/register"
```

Calling protected endpoint (passing Bearer Token) example:

```shell
curl -X GET -H "Accept: application/json" -H "Authorization: Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..." -H "http://http://api.url-shortener.dev/v1/users"
```
